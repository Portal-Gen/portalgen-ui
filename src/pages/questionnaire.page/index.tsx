...
const FavouriteTypeForm = () => {
  const STARTING_PLACE_TYPES_VALUE = 0.75;
  ...
  const onUpdateUserPlacePreferences = (data) => {
    const preferences = placeTypeOptions.reduce((acc, option) => {
      acc[PlaceType[option.label]] = data.placeTypes.includes(option.label)
        ? STARTING_PLACE_TYPES_VALUE
        : 0;
      return acc;
    }, {});
  ...
  return (
    <form
      onSubmit={handleSubmit(onUpdateUserPlacePreferences)}
    ...
    >
      ...
      <Controller
        ...
        rules={{
          validate: (value) =>
            value.length >= 5 || "Please select at least 5 types",
        }}
        render={({ field }) => (
          <Group style={{ marginBottom: "1rem" }}>
            {placeTypeOptions.map((option) => (
              <Pill
                ...
                style={{
                  cursor: "pointer",
                  backgroundColor: field.value.includes(option.label)
                    ? "darkblue"
                    : "lightgray",
